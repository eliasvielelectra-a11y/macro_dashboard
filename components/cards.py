...
def render_snapshot_line(indicator: Dict[str, Any], decimals: int = 2) -> str:
    snapshot_value = indicator.get("current_snapshot_value")
    snapshot_asof = indicator.get("current_snapshot_asof")
    unit = indicator.get("unit", "")

    if snapshot_value is None or pd.isna(snapshot_value):
        return "Current official snapshot: N/A"

    value_text = format_main_value(snapshot_value, unit=unit, decimals=decimals)
    if snapshot_asof:
        return f"Current official snapshot ({snapshot_asof}): {value_text}"
    return f"Current official snapshot: {value_text}"
...
def render_official_card(
    ...
    if policy.get("show_yoy", False):
        meta_lines.append(f'<div class="card-meta">{render_yoy_line(indicator, decimals=decimals)}</div>')

    if policy.get("show_snapshot", False):
        meta_lines.append(f'<div class="card-meta">{render_snapshot_line(indicator, decimals=decimals)}</div>')

    meta_lines.append(f'<div class="card-meta">{render_status_line(indicator)}</div>')
    ...
    with st.container(border=True):
        ...
        st.markdown(
            ...
            <div class="card-secondary {delta_class}">{compare_text}{pct_text}</div>
            {''.join(meta_lines)}
            """,
            unsafe_allow_html=True,
        )
        ...
        if snapshot_note:
            st.markdown(f'<div class="card-note">{snapshot_note}</div>', unsafe_allow_html=True)
